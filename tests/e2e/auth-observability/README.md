...
이 폴더는 auth-service의 Redis 관측성을 실제 Docker 컨테이너로 검증한다. 기능 E2E와 별도로 PostgreSQL, Valkey, OpenTelemetry Collector, Tempo, Prometheus, redis_exporter, Loki, Alloy, Grafana를 올리고, 자동 판정 스크립트가 cache miss, cache hit, Redis 장애와 복구를 확인한다.

Trace, metric, log 원본 판정은 Tempo, Prometheus, Loki API가 담당한다. Grafana는 이 데이터를 따로 저장하지 않고 세 backend에 질의하는 화면이다. Smoke 스크립트는 Grafana `/api/ds/query`도 호출해 provision된 세 datasource가 같은 데이터를 실제로 조회할 수 있는지 자동 확인한다.
...
다음 명령 하나가 stack 준비, fixture 생성, miss·hit·장애 검증, Redis 복구 확인, 자원 정리를 모두 수행한다.
...
2. cache miss 요청에서 Redis `HGETALL` miss, PostgreSQL `SELECT`, Redis `EVAL` write-through가 같은 trace에 차례로 있고 응답이 성공하는지 확인한다.
3. cache hit 요청에서 Redis `HGETALL`이 저장된 답을 바로 찾고 같은 trace에 PostgreSQL 조회나 Redis write-through가 없는지 확인한다.
4. Redis 중단 시 `/internal/ext-authz`가 fail-closed 응답을 반환하고 Redis error span과 `/readyz` 실패가 생기는지 확인한다.
5. Redis를 다시 시작한 뒤 `/readyz`와 인증 요청이 정상으로 돌아오는지 확인한다.
6. 응답 `X-Trace-Id`, Tempo trace ID, `service.name=auth-service`, 고유 `request_id`, HTTP parent-child 관계를 대조한다.
7. `/healthz`, `/readyz`, `/metrics`가 trace에서 제외되는지 확인한다.
8. auth-service Redis client metric, `redis_up`, 명령 처리량, keyspace hit·miss 변화가 Prometheus에 들어오는지 확인한다.
9. 같은 `request_id`와 `trace_id`, route, status를 가진 JSON access log를 Loki에서 찾는다.
10. Grafana에 provision된 Tempo, Prometheus, Loki datasource 계약을 확인하고 `/api/ds/query` 응답에 실제 data frame이 있는지 판정한다.
...
Grafana는 `http://127.0.0.1:<AUTH_OBS_GRAFANA_PORT>`에서 연다. 포트를 지정하지 않았다면 `docker-compose.yml`에 선언된 기본값을 사용한다. Tempo, Loki, Prometheus datasource는 미리 provision된다. Smoke는 세 datasource의 API 조회를 자동 판정하고, Explore 화면 확인은 사람이 결과를 더 자세히 살펴볼 때 사용한다.

### Tempo에서 Trace 찾기

1. 왼쪽 메뉴에서 **Explore**를 열고 datasource로 **Tempo**를 선택한다.
2. smoke 결과의 `X-Trace-Id`를 Trace ID 입력란에 붙여 넣는다.
3. HTTP server span 아래에 Redis `hgetall` span이 있고, miss일 때만 PostgreSQL `SELECT`와 Redis `eval` write-through span이 추가되는지 확인한다.
...
auth-service의 Redis OpenTelemetry client metric은 `db_client_connections_use_time_milliseconds_count`다. 이 시나리오에서 `HGETALL`과 `EVAL`은 모두 `type="command"`이며, 정상 명령은 `status="ok"`, Redis 장애 명령은 `status="error"`다. Cache miss는 정상 command가 두 번 이상 늘고, hit는 한 번 이상 늘며, 정확한 miss·hit 구분은 같은 Tempo trace의 span 구성과 redis_exporter keyspace 수치를 함께 사용한다. 자동 smoke도 실제 `/metrics`에서 이 metric family를 확인한 뒤 같은 이름으로 판정한다.
...
- Grafana 화면은 수동 확인용이지만, Tempo·Prometheus·Loki datasource provisioning과 `/api/ds/query` 조회 성공은 자동 성공 조건에 포함된다.
