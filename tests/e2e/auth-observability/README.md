...
11. Grafana에 provision된 Tempo, Prometheus, Loki datasource 계약을 확인하고 `/api/ds/query` 응답에 실제 data frame이 있는지 판정한다.
12. 수집된 trace와 log, 실패 출력에 테스트 인증정보나 세션 Redis key·value가 없는지 검사한다.
...
3. HTTP server span 아래에 Redis `get` span이 있고, miss일 때만 PostgreSQL `SELECT`, Redis `evalsha|eval` CAS, reverse-index pipeline span이 추가되는지 확인한다.
