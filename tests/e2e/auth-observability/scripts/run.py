...
def _trace_is_complete(
    ...
    if mode == "miss":
        return bool(_postgres_spans(spans)) and any(
            span.name.startswith("redis.pipeline ") for span in redis_spans
        )
    return True
...
def _validate_trace(
    ...
    if mode == "miss":
        ...
        write_pipelines = [
            ...
            if all(command in span.name for command in ("set", "sadd", "expire"))
        ]
        if not selects:
            raise CheckFailure("Tempo", "cache miss trace에 PostgreSQL 조회가 없습니다")
        if len(write_pipelines) != 1:
            raise CheckFailure("Tempo", "cache miss trace에 Redis write-through가 없습니다")
        pipeline = write_pipelines[0]
        if pipeline.parent_span_id != server.span_id:
            raise CheckFailure("Tempo", "Redis write-through가 HTTP span의 직접 자식이 아닙니다")
        if int(pipeline.attributes.get("db.redis.num_cmd", 0)) != 6:
            raise CheckFailure("Tempo", "Redis write-through 명령 구조가 예상과 다릅니다")
        if all(span.start_ns for span in (get_spans[0], selects[0], pipeline)) and not (
            get_spans[0].start_ns <= selects[0].start_ns <= pipeline.start_ns
        ):
...
def _metric_snapshot(settings: Settings, not_before: float) -> MetricSnapshot:
    ...
    return MetricSnapshot(
        ...
        pipeline_ok=_prometheus_value(settings, _client_metric_query("pipeline", "ok")),
    )
...
def _run_smoke(settings: Settings) -> None:
    ...
    miss_metrics = _wait_metrics(
        settings,
        time.time(),
        "cache miss/client nil/write-through/server miss metric 증가가 확인되지 않았습니다",
        lambda current: (
            current.client_nil > baseline.client_nil
            and current.pipeline_ok > baseline.pipeline_ok
    ...
    hit_metrics = _wait_metrics(
        ...
        lambda current: (
            ...
            and current.pipeline_ok == miss_metrics.pipeline_ok
        ),
    )
    ...
    print(
        "[성공] Tempo trace, Auth/Redis metric, Loki log, 장애 복구, 민감정보 비노출을 모두 확인했습니다",
        flush=True,
    )
