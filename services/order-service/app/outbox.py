...
class OutboxRelay:
    ...
    async def relay_once(self, now: datetime | None = None) -> bool:
        """Publish at most one due row while holding its database lock."""
        attempted_at = now or datetime.now(UTC)
        outcome: OutboxRelayOutcome
        async with self._session_factory.begin() as session:
            result = await session.execute(_claim_statement(attempted_at))
    ...
    async def run(self) -> None:
        """Relay pending rows until the enclosing task group cancels the worker."""
        while True:
...
def retry_delay(attempt: int) -> timedelta:
    """Return the bounded delay after a failed relay attempt."""
