...
func RegisterRoutes(mux *http.ServeMux, service coupon.Service, registry *metrics.Registry) {
...
func (h Handler) PreparePolicy(w http.ResponseWriter, r *http.Request) {
	var input coupon.PreparePolicyInput
...
func couponOutcome(err error) string {
	switch {
	case errors.Is(err, coupon.ErrSoldOut):
		return "sold_out"
	case errors.Is(err, coupon.ErrPolicyNotReady):
		return "not_ready"
	case errors.Is(err, coupon.ErrPolicyNotFound):
		return "not_found"
	case errors.Is(err, coupon.ErrUnauthorized):
		return "unauthorized"
	default:
		return "failed"
	}
...
func mapCouponError(err error) error {
	switch {
	case errors.Is(err, coupon.ErrInvalidPolicy):
		return httpapi.BadRequest("coupon.invalid_policy", "쿠폰 정책 값이 올바르지 않습니다.")
	case errors.Is(err, coupon.ErrInvalidIssue):
