...
func TestUserPostgresEnsureIsIdempotent(t *testing.T) {
	...
	store, err := user.OpenPostgresRepository(ctx, database.DefaultPostgresConfig(dsn))
	if err != nil {
		t.Fatalf("store open: %v", err)
	}
	svc := user.NewService(store)
	if _, err := svc.Ensure(ctx, user.EnsureInput{UserID: "user-pg-1"}); err != nil {
		t.Fatalf("Ensure() error = %v", err)
	}
	realName := "PG User"
	nickname := "pg-user"
	if _, err := svc.UpdateMyProfile(ctx, principal.Principal{Type: principal.TypeUser, UserID: "user-pg-1"}, user.ProfileUpdate{RealName: &realName, Nickname: &nickname}); err != nil {
