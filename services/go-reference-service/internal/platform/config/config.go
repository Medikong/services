...
const ServiceName = "go-reference-service"

var configErr = oops.In("reference_config").Code("config.invalid")
...
func (c ProfileConfig) Validate() error {
	return validation.ValidateStruct(&c,
		validation.Field(
			&c.PyroscopeAddress,
			validation.When(c.PyroscopeEnabled, validation.Required),
		),
	)
}
...
func (c AuditConfig) Validate() error {
	return validation.ValidateStruct(&c,
		...
		validation.Field(
			&c.Lease,
			validation.Min(2*c.PublishTimeout).Error("must be at least twice PublishTimeout"),
		),
...
func loadService() ServiceConfig {
	return ServiceConfig{
		...
		Environment: stringEnv("SERVICE_ENVIRONMENT", "dev"),
	}
}
