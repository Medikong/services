...
fn password_verify_api_contract() {
    ...
    let mut server = Command::new(env!("CARGO_BIN_EXE_medikong-auth-passwordhash-rust-server"))
        .args(["--addr", &addr])
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .expect("start rust benchmark server");
...
fn wait_for_health(addr: &str) {
    ...
    panic!("rust benchmark server did not become healthy");
}
