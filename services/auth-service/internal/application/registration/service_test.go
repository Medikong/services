...
func TestStartUsesConfiguredPasswordMinimumLength(t *testing.T) {
	service := &Service{config: Config{PasswordMinLength: 16}}
	_, err := service.Start(context.Background(), StartInput{
		Email: "user@example.test", Phone: "+821012345678", Password: "123456789012",
		ProfileRequestID: "profile", AgreementReceiptID: "agreement", IdempotencyKey: "key",
	})
	assertFailureCode(t, err, "AUTH_INPUT_INVALID")
}
...
func (c cryptographyStub) EqualHash([]byte, []byte) bool     { return c.equal }
