...
func NewService(pool *pgxpool.Pool, keys security.Keys, config Config, bootstrap *bootstrap.Service, resets resetdomain.Repository, identities identity.Repository, challenges challenge.Repository, idempotency idempotency.Repository, sessions sessiondomain.Repository, outbox outbox.Repository) *Service {
...
func (s *Service) Start(ctx context.Context, input StartInput) (StartOutput, error) {
	if strings.TrimSpace(input.IdempotencyKey) == "" {
		return StartOutput{}, application.Problem(400, "AUTH_INPUT_INVALID", "Idempotency-Key 헤더가 필요합니다.")
	}
	intentID, err := uuid.Parse(input.IntentID)
	if err != nil {
		return StartOutput{}, application.Problem(404, "AUTH_INTENT_NOT_FOUND", "인증 요청을 찾을 수 없습니다.")
	}
	...
	if err != nil {
		return StartOutput{}, application.Problem(400, "AUTH_INPUT_INVALID", "재설정 식별자가 올바르지 않습니다.")
	}
	...
	if err == nil {
		...
		if errors.Is(findErr, resetdomain.ErrNotFound) {
			return StartOutput{}, application.Problem(410, "AUTH_INTENT_EXPIRED", "비밀번호 재설정 요청 시간이 만료되었습니다.")
		}
		if findErr != nil {
			return StartOutput{}, application.Unavailable()
		}
	...
	resetID := uuid.New()
	reset, err := resetdomain.New(resetID, &currentIntent.ID, identityID, expires, now)
	if err != nil {
		return StartOutput{}, application.Unavailable()
	}
	...
	if err := application.AppendAudit(ctx, tx, "auth.password_reset.requested", "authentication_intent", currentIntent.ID, resetID, map[string]string{"status": "accepted"}, input.IdempotencyKey); err != nil {
		return StartOutput{}, application.Unavailable()
	}
	if err := tx.Commit(ctx); err != nil {
		return StartOutput{}, application.Unavailable()
...
func (s *Service) Issue(ctx context.Context, input IssueInput) (IssueOutput, error) {
	...
	if err != nil || strings.TrimSpace(input.IdempotencyKey) == "" {
		return IssueOutput{}, application.Problem(400, "AUTH_INPUT_INVALID", "재설정 Challenge 요청이 올바르지 않습니다.")
	}
	method := identity.Type(strings.TrimSpace(input.Method))
	if method != identity.TypeEmail && method != identity.TypePhone {
		return IssueOutput{}, application.Problem(400, "AUTH_INPUT_INVALID", "확인 수단이 올바르지 않습니다.")
	}
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return IssueOutput{}, application.Unavailable()
	}
	defer func() { _ = tx.Rollback(context.WithoutCancel(ctx)) }()
	reset, err := s.resets.FindForUpdate(ctx, tx, resetID)
	if errors.Is(err, resetdomain.ErrNotFound) {
		return IssueOutput{}, application.Problem(400, "AUTH_PASSWORD_RESET_INVALID", "비밀번호 재설정 요청이 올바르지 않습니다.")
	}
	if err != nil {
		return IssueOutput{}, application.Unavailable()
	}
	...
	if reset.Status != resetdomain.StatusRequested || !reset.ExpiresAt.After(time.Now()) {
		return IssueOutput{}, application.Problem(400, "AUTH_PASSWORD_RESET_INVALID", "비밀번호 재설정 요청이 올바르지 않습니다.")
	}
	...
	if reset.IdentityID != nil {
		...
		if target.Type != method {
			return IssueOutput{}, application.Problem(400, "AUTH_PASSWORD_RESET_INVALID", "선택한 확인 수단을 사용할 수 없습니다.")
		}
	...
	if err := application.AppendAudit(ctx, tx, "auth.password_reset.challenge_issued", "authentication_intent", *reset.IntentID, resetID, map[string]string{"method": string(method)}, input.IdempotencyKey); err != nil {
		return IssueOutput{}, application.Unavailable()
	}
	if err := tx.Commit(ctx); err != nil {
		return IssueOutput{}, application.Unavailable()
...
func (s *Service) Verify(ctx context.Context, input VerifyInput) (VerifyOutput, error) {
	...
	if err != nil || len(strings.TrimSpace(input.Code)) != 6 {
		return VerifyOutput{}, application.Problem(400, "AUTH_INPUT_INVALID", "확인 코드가 올바르지 않습니다.")
	}
	challengeID, err := uuid.Parse(input.ChallengeID)
	if err != nil {
		return VerifyOutput{}, application.Problem(400, "AUTH_INPUT_INVALID", "Challenge 식별자가 올바르지 않습니다.")
	}
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return VerifyOutput{}, application.Unavailable()
	}
	defer func() { _ = tx.Rollback(context.WithoutCancel(ctx)) }()
	reset, err := s.resets.FindForUpdate(ctx, tx, resetID)
	if errors.Is(err, resetdomain.ErrNotFound) || reset.IntentID == nil {
		return VerifyOutput{}, application.Problem(400, "AUTH_PASSWORD_RESET_INVALID", "비밀번호 재설정 요청이 올바르지 않습니다.")
	}
	if err != nil {
		return VerifyOutput{}, application.Unavailable()
	}
	...
	if !result.Verified {
		...
		if result.Failure == challenge.ConsumeFailureExpired {
			return VerifyOutput{}, application.Problem(410, "AUTH_CHALLENGE_EXPIRED", "확인 코드가 만료되었습니다.")
		}
		return VerifyOutput{}, application.Problem(400, "AUTH_CHALLENGE_FAILED", "확인 코드가 올바르지 않습니다.")
	}
	if reset.IdentityID == nil {
		if err := tx.Commit(ctx); err != nil {
			return VerifyOutput{}, application.Unavailable()
	...
	if reset.Status == resetdomain.StatusRequested {
		if err := reset.Verify(s.keys.Hash(resetID.String(), grant), time.Now().UTC()); err != nil {
			return VerifyOutput{}, application.Problem(400, "AUTH_PASSWORD_RESET_INVALID", "비밀번호 재설정 요청이 올바르지 않습니다.")
		}
	} else if reset.Status == resetdomain.StatusChallengeVerified {
	...
	if err := application.AppendAudit(ctx, tx, "auth.password_reset.verified", "authentication_intent", *reset.IntentID, resetID, map[string]string{"status": "verified"}, stableKey(input.IdempotencyKey, "verify-reset", challengeID)); err != nil {
		return VerifyOutput{}, application.Unavailable()
	}
	if err := tx.Commit(ctx); err != nil {
		return VerifyOutput{}, application.Unavailable()
...
func (s *Service) Complete(ctx context.Context, input CompleteInput) error {
	...
	if err != nil || input.NewPassword != input.ConfirmPassword || strings.TrimSpace(input.IdempotencyKey) == "" {
		return application.Problem(400, "AUTH_INPUT_INVALID", "비밀번호 재설정 요청이 올바르지 않습니다.")
	}
	if err := (security.PasswordPolicy{}).Validate(input.NewPassword); err != nil {
		return application.Problem(422, "AUTH_PASSWORD_POLICY_NOT_MET", "비밀번호 정책을 만족하지 않습니다.")
	}
	tx, err := s.pool.Begin(ctx)
	if err != nil {
	...
	defer func() { _ = tx.Rollback(context.WithoutCancel(ctx)) }()
	reset, err := s.resets.FindForUpdate(ctx, tx, resetID)
	if errors.Is(err, resetdomain.ErrNotFound) || reset.IntentID == nil || reset.IdentityID == nil {
		return application.Problem(400, "AUTH_PASSWORD_RESET_INVALID", "비밀번호 재설정 요청이 올바르지 않습니다.")
	}
	if err != nil {
		return application.Unavailable()
	}
	...
	if reset.Status != resetdomain.StatusChallengeVerified || !reset.ExpiresAt.After(time.Now()) {
		return application.Problem(410, "AUTH_PASSWORD_RESET_GRANT_EXPIRED", "비밀번호 재설정 권한이 만료되었습니다.")
	}
	if input.Channel == "mobile" && !s.keys.Equal(reset.ResetGrantHash, resetID.String(), input.ResetGrant) {
		return application.Problem(410, "AUTH_PASSWORD_RESET_GRANT_EXPIRED", "비밀번호 재설정 권한이 만료되었습니다.")
	}
	...
	if err := application.AppendAudit(ctx, tx, "auth.password_reset.completed", "authentication_intent", *reset.IntentID, resetID, map[string]string{"status": "completed"}, input.IdempotencyKey); err != nil {
		return application.Unavailable()
	}
