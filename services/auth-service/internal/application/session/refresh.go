...
type refreshOutcome struct {
	...
	fence         domainsession.RevocationFence
}
...
func (s *Service) Refresh(ctx context.Context, refreshToken, csrfToken, idempotencyKey string) (TokenSet, error) {
	...
	if outcome.fence != nil {
		if resolveErr := outcome.fence.Resolve(context.WithoutCancel(ctx)); resolveErr != nil {
			return TokenSet{}, unavailable(resolveErr)
		}
	}
	if err != nil {
		return TokenSet{}, unavailable(err)
	}
	if outcome.fence == nil && outcome.revokeSession != uuid.Nil && s.projection != nil {
...
func (s *Service) refreshTx(ctx context.Context, repositories TxRepositories, refreshToken, csrfToken, idempotencyKey string, outcome *refreshOutcome) error {
	...
	if credential.Status != "active" || current.Status != "active" || !credential.ExpiresAt.After(now) {
		if credential.FamilyID != nil {
			if s.revocations != nil && current.Status == "active" {
				var fenceErr error
				outcome.fence, fenceErr = s.revocations.Fence(ctx, []domainsession.Session{current})
				if fenceErr != nil {
