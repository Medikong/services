...
func NewEmailService(pool *pgxpool.Pool, bootstrap *bootstrap.Service, identities identity.Repository, intents intent.Repository, sessions *appsession.Service) *EmailService {
...
func (s *EmailService) SignIn(ctx context.Context, input EmailInput) (Completed, error) {
	...
	if !strings.Contains(email, "@") || strings.TrimSpace(input.Password) == "" {
		return Completed{}, application.Problem(400, "AUTH_INPUT_INVALID", "이메일과 비밀번호가 필요합니다.")
	}
	intentID, err := uuid.Parse(input.IntentID)
	if err != nil {
		return Completed{}, application.Problem(400, "AUTH_INPUT_INVALID", "인증 Intent 식별자가 올바르지 않습니다.")
	}
	...
	if errors.Is(err, identity.ErrNotFound) {
		return Completed{}, application.Problem(401, "AUTH_SIGNIN_FAILED", "이메일 또는 비밀번호가 올바르지 않습니다.")
	}
	if err != nil {
		return Completed{}, application.Unavailable()
	}
	if currentIdentity.CredentialState == "locked" {
		return Completed{}, application.Problem(423, "AUTH_ACCOUNT_LOCKED", "인증 식별자가 잠겨 있습니다.")
	}
	if currentIdentity.CredentialState == "password_reset_required" {
		return Completed{}, application.Problem(403, "AUTH_PASSWORD_RESET_REQUIRED", "비밀번호 재설정이 필요합니다.")
	}
	...
	issued, err := s.sessions.IssueTx(ctx, tx, appsession.IssueInput{
		...
		Method: "email_password", Channel: string(currentIntent.Channel), RememberMe: input.RememberMe,
	})
	if err != nil {
		return Completed{}, err
	}
