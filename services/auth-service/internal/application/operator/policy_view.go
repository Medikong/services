...
// PolicyView is the public active global policy snapshot. Storage keeps each
// policy bundle independently, but the operator API always returns the full
// decision surface that an administrator needs to inspect before changing it.
...
func policyView(items []policy.Snapshot) (PolicyView, error) {
	...
	if view.Version == 0 || view.LoginLock == nil || view.SessionTTL == nil || view.RefreshRotation == nil || len(view.VerificationRules) == 0 || len(view.SessionRevocationRules) == 0 {
		return PolicyView{}, fmt.Errorf("active policy snapshot is incomplete")
	}
	return view, nil
}

func policyViewFromGlobal(snapshot policy.GlobalSnapshot) (PolicyView, error) {
	var document struct {
	...
	view := PolicyView{
		...
		LoginLock:              document.LoginLock,
		SessionTTL:             document.SessionTTL,
		RefreshRotation:        document.RefreshRotation,
		VerificationRules:      document.VerificationRules,
		SessionRevocationRules: document.SessionRevocationRules,
	}
	if view.Version == 0 || view.Status != "active" || view.LoginLock == nil || view.SessionTTL == nil || view.RefreshRotation == nil || len(view.VerificationRules) == 0 || len(view.SessionRevocationRules) == 0 {
		return PolicyView{}, fmt.Errorf("active global policy snapshot is incomplete")
	}
	return view, nil
}
...
func (v PolicyView) document() (json.RawMessage, error) {
	return json.Marshal(map[string]any{
		...
		"sessionTtl":             v.SessionTTL,
		"refreshRotation":        v.RefreshRotation,
		"verificationRules":      v.VerificationRules,
		"sessionRevocationRules": v.SessionRevocationRules,
	})
}
