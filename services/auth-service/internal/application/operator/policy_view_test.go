...
func TestPolicyViewFromGlobalSnapshotPreservesSingleVersion(t *testing.T) {
	...
	view, err := policyViewFromGlobal(policy.GlobalSnapshot{Version: 8, Status: "active", EffectiveAt: effectiveAt, Document: document})
	if err != nil {
	...
	if len(encoded) == 0 {
		t.Fatal("expected complete policy document")
	}
}
