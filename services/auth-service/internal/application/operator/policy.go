...
func (s *Service) replayPolicyUpdate(ctx context.Context, idempotency IdempotencyRepository, replayID uuid.UUID, input PolicyUpdateInput) (PolicyUpdateOutput, error) {
	...
	if err != nil || payload.Kind != "operator_policy_update" || payload.DestroyedAt != nil || !payload.ExpiresAt.After(s.clock.Now().UTC()) || !hmac.Equal(payload.BindingHash, s.policyReplayBinding(input)) {
