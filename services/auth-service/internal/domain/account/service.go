...
type RepositoryFactory func(exec database.Executor) Repositories

type Transactor interface {
	WithTx(ctx context.Context, fn func(database.Executor) error) error
}
...
func (s Service) Signup(ctx context.Context, input SignupInput) (principal.AuthResult, error) {
	...
	err = s.transactor.WithTx(ctx, func(exec database.Executor) error {
		...
		authAccountID := database.NewID("auth")
