...
func NewVirtualMessageService(pool *pgxpool.Pool, keys security.Keys, bootstrap *intent.BootstrapService, challenges challenge.Repository, registrations registrationdomain.Repository, resets resetdomain.Repository, identities identity.Repository) *VirtualMessageService {
...
func (s *VirtualMessageService) Get(ctx context.Context, input VirtualMessageInput) (VirtualMessageOutput, error) {
	...
	if err != nil {
		return VirtualMessageOutput{}, domain.Unavailable()
	}
	...
	if errors.Is(err, challenge.ErrVirtualUnavailable) {
		return VirtualMessageOutput{}, domain.Problem(410, "AUTH_VIRTUAL_MESSAGE_UNAVAILABLE", "가상 인증 메시지를 더 이상 사용할 수 없습니다.")
	}
	if err != nil {
		return VirtualMessageOutput{}, domain.Unavailable()
	}
...
func virtualNotFound() error {
	return domain.Problem(404, "AUTH_VIRTUAL_MESSAGE_NOT_FOUND", "가상 인증 메시지를 찾을 수 없습니다.")
}
