...
import (
	...
	"github.com/Medikong/services/services/auth-service/internal/domain/autherror"
...
func (r PostgresRepository) create(ctx context.Context, input Input, accessToken string, refreshToken string) (Record, error) {
	...
	_, err := r.exec.ExecContext(ctx, `
		...
		record.SessionID, record.AuthAccountID, record.UserID, record.AccessToken, database.HashToken(record.RefreshToken), strings.Join(record.AuthMethods, ","))
	if err != nil {
		return Record{}, err
	}
	return record, nil
...
func (r PostgresRepository) FindByAccessToken(ctx context.Context, token string) (Record, error) {
	...
	record.AuthMethods = database.SplitCSV(methods)
	return record, nil
}

func (r PostgresRepository) Refresh(ctx context.Context, refreshToken string) (Rotation, error) {
	newAccessToken := "atk_" + database.RandomHex(24)
	newRefreshToken := "rtk_" + database.RandomHex(24)
	row := r.exec.QueryRowContext(ctx, `
		WITH current AS (
			SELECT session_id, access_token, auth_account_id, user_id, auth_methods
			FROM auth_sessions
		...
		FROM updated`,
		database.HashToken(refreshToken), newAccessToken, database.HashToken(newRefreshToken))
