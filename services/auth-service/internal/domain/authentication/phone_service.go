...
func NewPhoneService(pool *pgxpool.Pool, keys security.Keys, bootstrap *intent.BootstrapService, intents intent.Repository, identities identity.Repository, challenges challenge.Repository, outbox outbox.Repository, sessions *appsession.Service, virtual bool, challengeTTL time.Duration) *PhoneService {
...
func (s *PhoneService) Issue(ctx context.Context, input PhoneIssueInput) (PhoneIssueOutput, error) {
	...
	if err != nil || strings.TrimSpace(input.IdempotencyKey) == "" {
		return PhoneIssueOutput{}, domain.Problem(400, "AUTH_INPUT_INVALID", "휴대폰 로그인 요청이 올바르지 않습니다.")
	}
	phone, err := normalizePhone(input.Phone)
	if err != nil {
		return PhoneIssueOutput{}, domain.Problem(400, "AUTH_INPUT_INVALID", "휴대폰 번호 형식이 올바르지 않습니다.")
	}
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return PhoneIssueOutput{}, domain.Unavailable()
	}
	...
	if err := domain.AppendAudit(ctx, tx, "auth.phone_signin.challenge_issued", "authentication_intent", intentID, intentID, map[string]string{"status": "accepted"}, input.IdempotencyKey); err != nil {
		return PhoneIssueOutput{}, domain.Unavailable()
	}
...
func (s *PhoneService) Verify(ctx context.Context, input PhoneVerifyInput) (Completed, error) {
	...
	if err != nil || len(strings.TrimSpace(input.Code)) != 6 {
		return Completed{}, domain.Problem(400, "AUTH_INPUT_INVALID", "확인 코드가 올바르지 않습니다.")
	}
	challengeID, err := uuid.Parse(input.ChallengeID)
	if err != nil {
		return Completed{}, domain.Problem(400, "AUTH_INPUT_INVALID", "Challenge 식별자가 올바르지 않습니다.")
	}
	tx, err := s.pool.Begin(ctx)
	if err != nil {
	...
	if verification.SubjectType != challenge.SubjectPhoneSignIn || verification.SubjectID != intentID || verification.IdentityID == nil {
		return Completed{}, domain.Problem(401, "AUTH_SIGNIN_FAILED", "휴대폰 로그인 정보를 확인할 수 없습니다.")
	}
	if !result.Verified {
		if err := tx.Commit(ctx); err != nil {
			return Completed{}, domain.Unavailable()
		...
		if result.Failure == challenge.ConsumeFailureExpired {
			return Completed{}, domain.Problem(410, "AUTH_CHALLENGE_EXPIRED", "확인 코드가 만료되었습니다.")
		}
		return Completed{}, domain.Problem(400, "AUTH_CHALLENGE_FAILED", "확인 코드가 올바르지 않습니다.")
	}
	link, err := s.identities.FindActiveLinkForIdentity(ctx, tx, *verification.IdentityID)
	if errors.Is(err, identity.ErrNotFound) {
		return Completed{}, domain.Problem(409, "AUTH_PHONE_IDENTITY_NOT_LINKED", "연결된 휴대폰 인증 수단이 없습니다.")
	}
	if err != nil {
		return Completed{}, domain.Unavailable()
	}
	issued, err := s.sessions.IssueTx(ctx, tx, appsession.IssueInput{UserID: link.UserID, IdentityID: *verification.IdentityID, IdentityLink: link.ID, Method: "phone_otp", Channel: string(currentIntent.Channel), RememberMe: currentIntent.RememberMe != nil && *currentIntent.RememberMe, WebCSRFToken: input.CSRFToken})
	if err != nil {
		return Completed{}, err
	}
	...
	if err := domain.AppendAudit(ctx, tx, "auth.phone_signin.completed", "authentication_intent", intentID, intentID, map[string]string{"status": "completed"}, stableKey(input.IdempotencyKey, "phone-signin", challengeID)); err != nil {
		return Completed{}, domain.Unavailable()
	}
	if err := tx.Commit(ctx); err != nil {
		return Completed{}, domain.Unavailable()
