...
func NewService(pool *pgxpool.Pool, keys security.Keys, users Repository, policies policy.Repository, idempotency idempotency.Repository, outbox outbox.Repository, config Config, approvals ApprovalPort, decisions appstate.AuthorizationDecisionPort) *Service {
	if approvals == nil {
		approvals = DenyApprovalPort{}
	}
	if decisions == nil {
		decisions = appstate.DenyAuthorizationDecisionPort{}
	}
...
func (s *Service) User(ctx context.Context, principal appsession.Principal, decision, userID, reasonCode, auditKey string) (UserView, error) {
	...
	if !validAuditReason(reasonCode) {
		return UserView{}, domain.Problem(400, "AUTH_INPUT_INVALID", "감사 사유 코드가 올바르지 않습니다.")
	}
	id, err := uuid.Parse(userID)
	if err != nil {
		return UserView{}, domain.Problem(400, "AUTH_INPUT_INVALID", "사용자 식별자가 올바르지 않습니다.")
	}
	view, err := s.users.GetUser(ctx, id)
	if errors.Is(err, ErrNotFound) {
		return UserView{}, domain.Problem(404, "AUTH_OPERATOR_TARGET_NOT_FOUND", "운영 대상 인증 상태를 찾을 수 없습니다.")
	}
	if err != nil {
		return UserView{}, domain.Unavailable()
	}
	...
	if err := domain.AppendAudit(ctx, tx, "auth.operator_user.viewed", "operator", principal.UserID, id, map[string]string{"reasonCode": reasonCode}, auditKey); err != nil {
		return UserView{}, domain.Unavailable()
	}
...
func (s *Service) PolicyView(ctx context.Context, principal appsession.Principal, decision string) (PolicyView, error) {
	...
	if err != nil {
		return PolicyView{}, domain.Unavailable()
	}
...
func (s *Service) UpdatePolicy(ctx context.Context, input PolicyUpdateInput) (PolicyUpdateOutput, error) {
	...
	if name == "" {
		return PolicyUpdateOutput{}, domain.Problem(400, "AUTH_INPUT_INVALID", "정책 변경 요청이 올바르지 않습니다.")
	}
	if _, err := uuid.Parse(input.IdempotencyKey); err != nil {
		return PolicyUpdateOutput{}, domain.Problem(400, "AUTH_INPUT_INVALID", "Idempotency-Key는 UUID여야 합니다.")
	}
	...
	if dbPolicyName(bodyName) != name || strings.TrimSpace(reason) == "" {
		return PolicyUpdateOutput{}, domain.Problem(400, "AUTH_INPUT_INVALID", "정책 이름과 변경 사유를 확인해주세요.")
	}
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return PolicyUpdateOutput{}, domain.Unavailable()
	}
	defer func() { _ = tx.Rollback(context.WithoutCancel(ctx)) }()
	...
	if !claimed {
		if !hmac.Equal(record.RequestHash, requestHash) {
			return PolicyUpdateOutput{}, domain.Problem(409, "AUTH_IDEMPOTENCY_CONFLICT", "같은 멱등성 키를 다른 요청에 사용할 수 없습니다.")
		}
	...
	if err != nil || expected != global.Version {
		return PolicyUpdateOutput{}, domain.Problem(412, "AUTH_POLICY_PRECONDITION_FAILED", "정책 version이 현재 상태와 다릅니다.")
	}
	...
	if previous.Name == "" {
		return PolicyUpdateOutput{}, domain.Problem(400, "AUTH_INPUT_INVALID", "알 수 없는 정책입니다.")
	}
	raw, err := patchPolicyRules(name, previous.Rules, input.Patch)
	if err != nil {
		return PolicyUpdateOutput{}, domain.Problem(400, "AUTH_INPUT_INVALID", "정책 값이 계약을 만족하지 않습니다.")
	}
	...
	if err := domain.AppendAudit(ctx, tx, "auth.policy.updated", "operator", input.Principal.UserID, uuid.New(), map[string]string{"policy": name}, input.IdempotencyKey); err != nil {
		return PolicyUpdateOutput{}, domain.Unavailable()
	}
	if err := tx.Commit(ctx); err != nil {
		return PolicyUpdateOutput{}, domain.Unavailable()
...
func (s *Service) Manual(ctx context.Context, input ManualInput) (uuid.UUID, int64, error) {
	...
	if s.approvals.Verify(ctx, ApprovalRequest{CaseID: input.CaseID, ApprovalID: input.ApprovalID, EvidenceRef: input.EvidenceRef, Action: input.Action, TargetType: input.TargetType, TargetID: input.TargetID}) != nil {
		return uuid.Nil, 0, domain.Problem(409, "AUTH_APPROVAL_REQUIRED", "승인된 운영 작업이 필요합니다.")
	}
	if _, err := uuid.Parse(input.IdempotencyKey); err != nil {
		return uuid.Nil, 0, domain.Problem(400, "AUTH_INPUT_INVALID", "Idempotency-Key는 UUID여야 합니다.")
	}
	tx, err := s.pool.Begin(ctx)
	if err != nil {
	...
	if err == nil {
		if !hmac.Equal(record.RequestHash, requestHash) {
			return uuid.Nil, 0, domain.Problem(409, "AUTH_IDEMPOTENCY_CONFLICT", "같은 멱등성 키를 다른 요청에 사용할 수 없습니다.")
		}
...
func (s *Service) authorize(ctx context.Context, principal appsession.Principal, decision string, reauthRequired bool, action, resource string) error {
	if !principal.Authenticated || principal.UserID == uuid.Nil || principal.SessionID == uuid.Nil {
		return domain.Problem(403, "AUTH_FORBIDDEN", "유효한 운영자 Session이 필요합니다.")
	}
	if !strongSession(principal, s.strongTTL) {
		if reauthRequired {
			return domain.Problem(403, "AUTH_REAUTH_REQUIRED", "최근 강한 인증이 필요합니다.")
		}
		return domain.Problem(403, "AUTH_FORBIDDEN", "최근 강한 인증이 필요합니다.")
	}
	if strings.TrimSpace(decision) == "" {
		return domain.Problem(403, "AUTH_FORBIDDEN", "외부 인가 결정을 확인할 수 없습니다.")
	}
