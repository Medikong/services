...
import (
	...
	platformdatabase "github.com/Medikong/services/packages/go-platform/database"
)
...
func Open(ctx context.Context, databaseURL string) (*DB, error) {
	db, err := platformdatabase.OpenPostgres(ctx, databaseURL)
	if err != nil {
		return nil, err
	}
...
func (db *DB) Migrate(ctx context.Context, migrations []string) error {
	return platformdatabase.RunMigrations(ctx, db.SQL, migrations)
}
