...
func wireUseCases(cfg config.ServerConfig, options ServerOptions, adapters serverAdapters) (serverUseCases, error) {
	...
	passwordResetProjectionWriters := make([]applicationpasswordreset.StatusProjectionWriter, 0, 1)
	...
	if adapters.sessionProjection != nil {
		...
		passwordResetProjectionWriters = append(passwordResetProjectionWriters, adapters.sessionProjection)
	...
	passwordReset := applicationpasswordreset.NewService(
		...
		passwordResetProjectionWriters...,
	)
	...
	registration := applicationregistration.NewService(
		...
		applicationregistration.Config{
			...
			ChallengeTTL: cfg.Auth.ChallengeTTL, VirtualAdapterEnabled: cfg.Development.VirtualAdaptersEnabled,
		},
		bootstrap,
		sessions,
		authProofSigner,
