...
type DevelopmentController struct {
	...
	virtual     *applicationdevelopment.Service
	sessions    *appsession.Service
}

func NewDevelopment(credentials *httpauth.Credentials, virtual *applicationdevelopment.Service, sessions *appsession.Service) *DevelopmentController {
...
func (c *DevelopmentController) VirtualMessage(w http.ResponseWriter, r *http.Request) {
	if credentialErr := c.credentials.DevelopmentToken(r); credentialErr != nil {
		httputil.WriteError(w, r, virtualMessageNotFound())
		return
	}
	ownerProof, csrf := "", ""
	...
	result, err := c.virtual.GetVirtualMessage(r.Context(), applicationdevelopment.VirtualMessageInput{ChallengeID: chi.URLParam(r, "challengeId"), OwnerProof: ownerProof, CSRFToken: csrf, SessionUser: sessionUserID})
	if err != nil {
