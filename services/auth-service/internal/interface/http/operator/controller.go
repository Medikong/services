...
type OperatorController struct {
	...
	service     *applicationoperator.Service
}

func NewOperator(credentials *httpauth.Credentials, sessions *applicationsession.Service, service *applicationoperator.Service) *OperatorController {
...
func (c *OperatorController) User(w http.ResponseWriter, r *http.Request) {
	token, ok := c.mobileToken(w, r)
	if !ok {
		return
	}
	principal, err := c.sessions.Authenticate(r.Context(), "", token)
	if err != nil {
...
func (c *OperatorController) UpdatePolicy(w http.ResponseWriter, r *http.Request) {
	...
	updated, err := c.service.UpdatePolicy(r.Context(), applicationoperator.PolicyUpdateInput{Principal: principal, Name: chi.URLParam(r, "policyName"), IfMatch: r.Header.Get("If-Match"), IdempotencyKey: r.Header.Get("Idempotency-Key"), AuthorizationDecision: r.Header.Get("X-Authorization-Decision"), Patch: request})
	if err != nil {
		httputil.WriteError(w, r, err)
		return
	}
...
func (c *OperatorController) mobileToken(w http.ResponseWriter, r *http.Request) (string, bool) {
