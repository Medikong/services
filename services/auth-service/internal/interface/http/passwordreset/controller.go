...
type PasswordResetController struct {
	...
	csrf        *httputil.CSRF
	service     *applicationpasswordreset.Service
}

func NewPasswordReset(credentials *httpauth.Credentials, csrf *httputil.CSRF, service *applicationpasswordreset.Service) *PasswordResetController {
...
func (c *PasswordResetController) Start(w http.ResponseWriter, r *http.Request) {
	...
	result, err := c.service.Start(r.Context(), applicationpasswordreset.StartInput{IntentID: credential.IntentID, OwnerProof: credential.Token, CSRFToken: csrf, IdentifierType: request.IdentifierType, Email: request.Email, Phone: phone, IdempotencyKey: r.Header.Get("Idempotency-Key")})
	if err != nil {
...
func (c *PasswordResetController) Issue(w http.ResponseWriter, r *http.Request) {
	...
	result, err := c.service.Issue(r.Context(), applicationpasswordreset.IssueInput{ResetID: chi.URLParam(r, "passwordResetId"), OwnerProof: credential.Token, CSRFToken: csrf, Method: request.Method, IdempotencyKey: r.Header.Get("Idempotency-Key")})
	if err != nil {
		httputil.WriteError(w, r, err)
		return
	}
...
func (c *PasswordResetController) Verify(w http.ResponseWriter, r *http.Request) {
	...
	result, err := c.service.Verify(r.Context(), applicationpasswordreset.VerifyInput{ResetID: chi.URLParam(r, "passwordResetId"), ChallengeID: chi.URLParam(r, "challengeId"), OwnerProof: credential.Token, CSRFToken: csrf, Code: request.Code, Channel: channel, IdempotencyKey: r.Header.Get("Idempotency-Key")})
	if err != nil {
		httputil.WriteError(w, r, err)
		return
	}
...
func (c *PasswordResetController) preAuth(w http.ResponseWriter, r *http.Request) (httpauth.PreAuth, string, bool) {
	...
	if credential.IntentID == "" {
		httputil.WriteError(w, r, failure.NotFound("AUTH_INTENT_NOT_FOUND", "인증 요청을 찾을 수 없습니다."))
		return httpauth.PreAuth{}, "", false
	}
