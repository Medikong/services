...
func TestHashComparisonStaysInsideCryptographyAdapter(t *testing.T) {
	keys := Keys{CredentialHMAC: []byte("01234567890123456789012345678901")}
	expected := keys.Hash("idempotency", "request")
	if !keys.EqualHash(expected, append([]byte(nil), expected...)) {
		t.Fatal("equal hashes were rejected")
	}
	tampered := append([]byte(nil), expected...)
	tampered[0] ^= 1
	if keys.EqualHash(expected, tampered) {
		t.Fatal("different hashes were accepted")
	}
}
