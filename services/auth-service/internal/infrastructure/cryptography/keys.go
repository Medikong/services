// Package cryptography owns opaque credential, replay-payload, and JWT
...
func (k Keys) EqualHash(expected, actual []byte) bool {
...
func (k Keys) JWKS() (applicationjwks.Set, error) {
	active, err := k.verificationKey(k.JWTKeyID)
	if err != nil {
		return applicationjwks.Set{}, err
	}
	result := applicationjwks.Set{Keys: []applicationjwks.Key{rsaJWK(k.JWTKeyID, active)}}
