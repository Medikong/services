...
func NewSessionProjection(
	...
	if repository == nil || client == nil || timeout <= 0 || dbFallbackTimeout <= 0 || dbFallbackTimeout > timeout ||
		activeTTL <= 0 || tombstoneTTL < activeTTL || maxDBLookups <= 0 {
	...
	return &SessionProjection{
		...
		activeTTL: activeTTL, tombstoneTTL: tombstoneTTL, dbFallbacks: make(chan struct{}, maxDBLookups), now: time.Now,
	}, nil
}
...
func (s *SessionProjection) Check(ctx context.Context, userID, sessionID uuid.UUID) (bool, error) {
	...
	if err == nil {
		cached, decodeErr := s.decodeCached(encoded)
		if decodeErr != nil {
			return false, decodeErr
		}
		if cached.Status == "revoking" && cached.UserID == userID.String() && cached.SessionID == sessionID.String() {
			return s.reconcileCachedFence(checkCtx, cached, userID, sessionID)
		}
		return s.allowCached(cached, userID, sessionID), nil
	}
	if !errors.Is(err, goredis.Nil) {
		return false, oops.In("auth_session_status").Code("session_status.cache_unavailable").Wrap(err)
	}
	release, err := s.reserveDatabaseFallback()
	if err != nil {
		return false, err
	}
	defer release()
	current, err := s.findDatabaseStatus(checkCtx, sessionID, false)
...
func (s *SessionProjection) decodeCached(encoded []byte) (sessionStatusValue, error) {
	var cached sessionStatusValue
	if json.Unmarshal(encoded, &cached) != nil || cached.Version < 0 || cached.Status == "" || cached.SessionID == "" {
		return sessionStatusValue{}, oops.In("auth_session_status").Code("session_status.cache_invalid").New("Session status cache value is invalid")
	}
	if cached.Status == "revoking" && cached.FenceID == "" {
		return sessionStatusValue{}, oops.In("auth_session_status").Code("session_status.cache_invalid").New("Session status cache value is invalid")
	}
	return cached, nil
}

func (s *SessionProjection) allowCached(cached sessionStatusValue, userID, sessionID uuid.UUID) bool {
	return cached.Status == "active" && cached.UserID == userID.String() && cached.SessionID == sessionID.String() &&
		s.now().UTC().Before(cached.ExpiresAt)
}

func (s *SessionProjection) reconcileCachedFence(
	ctx context.Context,
	cached sessionStatusValue,
	...
	defer release()
	current, err := s.findDatabaseStatus(ctx, sessionID, true)
	if errors.Is(err, domainsession.ErrNotFound) {
		return false, nil
	}
	if err != nil {
	...
	allowed, err := s.resolveFenceFromStatus(ctx, sessionFenceEntry{sessionID: sessionID, fenceID: cached.FenceID}, current)
	if err != nil {
		return false, oops.In("auth_session_status").Code("session_status.fence_reconciliation_failed").Wrap(err)
	}
	return allowed, nil
}

func (s *SessionProjection) reserveDatabaseFallback() (func(), error) {
	select {
	case s.dbFallbacks <- struct{}{}:
		return func() { <-s.dbFallbacks }, nil
	default:
		return nil, oops.In("auth_session_status").Code("session_status.database_budget_exhausted").
			New("Session status database fallback budget is exhausted")
	}
}

func (s *SessionProjection) findDatabaseStatus(
	ctx context.Context,
	sessionID uuid.UUID,
	forReconciliation bool,
) (domainsession.Session, error) {
	dbCtx, cancel := context.WithTimeout(ctx, s.dbFallbackTimeout)
	defer cancel()
	var current domainsession.Session
	var err error
	if forReconciliation {
		current, err = s.repository.FindStatusForReconciliation(dbCtx, sessionID)
	} else {
	...
	if err != nil {
		return domainsession.Session{}, oops.In("auth_session_status").Code("session_status.database_unavailable").Wrap(err)
	}
	return current, nil
}
