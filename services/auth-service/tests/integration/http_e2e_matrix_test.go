...
var httpAPIContracts = []httpAPIContract{
	...
	{ID: "API.A.300-06", Environment: "production", Method: "POST", Path: "/api/v1/auth/registrations/{registrationId}/complete", SuccessStatuses: []string{"200", "202"}, ErrorStatus: "409", ErrorCode: "AUTH_VERIFICATION_REQUIRED"},
	...
	{ID: "API.A.300-16", Environment: "production", Method: "GET", Path: "/api/v1/auth/context", SuccessStatuses: []string{"200"}, ErrorStatus: "400", ErrorCode: "AUTH_MULTIPLE_CREDENTIALS"},
...
func TestHTTPAPIMatrixMatchesBundledOpenAPI(t *testing.T) {
	...
	if len(production) != 29 {
		t.Fatalf("production OpenAPI operation count = %d, want 29", len(production))
	}
	if len(development) != 1 {
		t.Fatalf("development OpenAPI operation count = %d, want 1", len(development))
	}
	seen := make(map[string]struct{}, len(httpAPIContracts))
	for _, contract := range httpAPIContracts {
		t.Run(contract.ID, func(t *testing.T) {
			if _, duplicate := seen[contract.ID]; duplicate {
				t.Fatalf("duplicate API matrix entry %s", contract.ID)
			}
			seen[contract.ID] = struct{}{}
			operations := production
			if contract.Environment == "development" {
				operations = development
			}
			operation, ok := operations[contract.Method+" "+contract.Path]
			if !ok {
				t.Fatalf("OpenAPI operation is missing for %s", contract.ID)
			}
			if operation.APIID != contract.ID {
				t.Fatalf("OpenAPI x-api-id = %q, want %q", operation.APIID, contract.ID)
			}
			for _, status := range append(append([]string(nil), contract.SuccessStatuses...), contract.ErrorStatus) {
				if _, ok := operation.Responses[status]; !ok {
					t.Errorf("OpenAPI response %s is missing for %s", status, contract.ID)
				}
			}
			wantErrorStatus, err := strconv.Atoi(contract.ErrorStatus)
			if err != nil {
				t.Fatalf("invalid matrix error status %q", contract.ErrorStatus)
			}
			if got, ok := operation.ErrorCodes[contract.ErrorCode]; !ok || got != wantErrorStatus {
				t.Errorf("OpenAPI error %s = %d, want %d", contract.ErrorCode, got, wantErrorStatus)
			}
		})
	}
	if len(seen) != 30 {
		t.Fatalf("API matrix entry count = %d, want 30", len(seen))
	}
