...
type sessionStatusFixture struct {
	userID     uuid.UUID
...
func newSessionStatusFixture(state string) sessionStatusFixture {
	return sessionStatusFixture{
		userID: uuid.New(), identityID: uuid.New(), linkID: uuid.New(), sessionID: uuid.New(), state: state,
	}
}

func (f sessionStatusFixture) seed(t *testing.T, ctx context.Context, db *pgxpool.Pool) {
	t.Helper()
...
func Test_SessionStatus_real_HTTP_Postgres_and_Redis_surface(t *testing.T) {
	// Given
	...
	active := newSessionStatusFixture("active")
	revoked := newSessionStatusFixture("revoked")
	unavailable := newSessionStatusFixture("active")
	for _, fixture := range []sessionStatusFixture{active, revoked, unavailable} {
	...
	activeResponse := harness.do(httpE2ERequest{
		...
		Path:   "/internal/session/status/orders/ord-1",
		Headers: http.Header{
	...
	require.Equal(t, []string{"X-Session-Id", "X-Token-Id", "X-User-Id"}, sessionStatusIdentityHeaders(activeResponse.header))
	assertResponseOmits(t, activeResponse, activeToken)

	// When
	invalidResponse := harness.do(httpE2ERequest{
		Method: http.MethodGet, Path: "/internal/session/status/catalog/items/1",
		Credentials: httpE2ECredentials{AccessToken: activeToken + "x"},
	})
	revokedResponse := harness.do(httpE2ERequest{
		Method: http.MethodDelete, Path: "/internal/session/status/payments/pay-1",
	...
	decodeHTTPError(t, revokedResponse, http.StatusUnauthorized, "AUTH_SESSION_REVOKED")
	require.Empty(t, sessionStatusIdentityHeaders(invalidResponse.header))
	require.Empty(t, sessionStatusIdentityHeaders(revokedResponse.header))
	...
	unavailableResponse := harness.do(httpE2ERequest{
		Method: http.MethodPatch, Path: "/internal/session/status/orders/ord-2",
	...
	require.Empty(t, sessionStatusIdentityHeaders(unavailableResponse.header))
...
func sessionStatusIdentityHeaders(header http.Header) []string {
