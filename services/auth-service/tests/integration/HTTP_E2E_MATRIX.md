...
- `common`은 기존 helper 테스트가 envelope, 공개 오류 응답, request ID, strict JSON, cookie/bearer, CSRF/Origin, no-store를 단위 수준에서 확인한다는 뜻이다.
- 새 E2E는 controller나 application service를 직접 호출하지 않고 실제 route와 middleware를 통과해야 한다.
- 운영 server에는 API.A.300-01~29와 API.A.300-31을 등록한다. API.A.300-30은 `SERVICE_ENVIRONMENT=test`와 개발 route, Virtual Adapter, access token, encryption key가 모두 유효한 별도 server에서만 확인한다.
- 성공 JSON은 `data/meta`, 오류는 `application/json`과 `status/code/message/requestId`, 모든 응답은 `X-Request-Id`와 `Cache-Control: no-store`를 확인한다. 204 응답은 body가 없어야 한다.
...
| API.A.300-06 | 운영 | `POST /api/v1/auth/registrations/{registrationId}/complete` | 200 | 409 `AUTH_VERIFICATION_REQUIRED` | route + common | User 서명 `userCreationProof`, IdentityLink/UserAuthState/Session 원자적 생성, idempotency |
...
| API.A.300-16 | 운영 | `GET /api/v1/auth/context` | 200 | 401 `AUTH_SESSION_REQUIRED` | route + common | access bearer 검증, Session/UserAuthState 확정, `Vary` |
...
| API.A.300-24 | 운영 | `GET /api/v1/operator/auth/users/{userId}` | 200 | 403 `AUTH_FORBIDDEN` | route + common | 서명된 외부 인가 결정, audit reason header, strict masked response와 민감 값 미노출 |
| API.A.300-25 | 운영 | `GET /api/v1/operator/auth/policies` | 200 | 403 `AUTH_FORBIDDEN` | route + common | active policy snapshot, strong `ETag`, 서명된 외부 인가 결정 |
...
| API.A.300-31 | 운영 | `PUT /api/v1/operator/auth/users/{userId}/account-status` | 200 | 403 `AUTH_FORBIDDEN` | route + common | User 상태 proof와 외부 인가 결정, version ordering, 대상 Session 폐기 |

## 시나리오 묶음

- 가입: 01 → 02 → 03 → 04 → 30 → 05 → User 생성 → 06 → 28
...
- 운영자/행동 재개: 24 → 25 → 26 → 27 → 31, 별도 인증 완료 뒤 29

각 묶음은 HTTP 응답으로 다음 단계의 opaque ID와 credential을 얻는다. DB 직접 조작은 정책 seed, 전달 실패 주입처럼 공개 HTTP API로 만들 수 없는 fixture 준비와 결과 확인에만 제한한다.
